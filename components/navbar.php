...
<!-- search flyout -->
<div class="search-flyout bg-light py-4" id="searchFlyout" style="z-index: 1050;">
  <div class="container">
    <div class="row justify-content-center">
      <div class="col-12 col-sm-10 col-md-8 col-lg-6">
        <div class="position-relative">
          <input type="text" class="form-control shadow-sm pe-5 rounded-4" placeholder="Search on Technologia..."
            id="searchInput">
          <button onclick="toggleSearch()" type="button"
            class="btn-close position-absolute top-50 end-0 translate-middle-y me-3" aria-label="Close">
          </button>
        </div>
      </div>
    </div>
...
<script>
  ...
  // Xử lý thay đổi URL (nút back/forward trình duyệt)
  ...
  // Khởi tạo các event listener khi DOM đã tải xong
  document.addEventListener('DOMContentLoaded', function () {
    // Xử lý URL ban đầu khi vào trang
    handleUrlChange();

    // Event listener cho ô tìm kiếm (xử lý phím Enter)
    const searchInput = document.getElementById('searchInput');
    if (searchInput) {
      searchInput.addEventListener('keydown', handleSearch);
    }

    // Đóng search flyout khi nhấn phím ESC
    document.addEventListener('keydown', function (event) {
      if (event.key === 'Escape' && isSearchOpen) {
        flyout = document.getElementById('searchFlyout');
        flyout.classList.remove('show');
        flyout.setAttribute('aria-hidden', 'true');
        isSearchOpen = false;
      }
  ...
  // Event listener cho nút back/forward trình duyệt
